								...
								<!-- <table class="table table-hover table-middle table-bordered text-right" id="saTable">
									...
									<tbody>
										...
										<tr v-for="(item,idx) in itemList">
											...
											<td>{{ item.sumCount | comma }}</td>

										</tr>
										<tr>
											<td align="center" colspan="2"><strong>합계</strong></td>
											<td><strong>{{ product1 | comma }}</strong></td>n
											...
											<td><strong>{{ totalCount | comma }}</strong></td>
...
<script>
import { GetSaOrderDt } from '@/api/index';

export default {
	data() {
		return {
			searchSDT: new Date().toISOString().slice(0, 7),
	...
	methods: {
		initData() {
			...
			GetSaOrderDt(params)
				.then(res => {
					...
					for(let i=0; i<this.itemList.length; i++) {
						...
						// }
						this.product1 += this.itemList[i]['productList'][0]['userCount']
						this.product2 += this.itemList[i]['productList'][1]['userCount']
						this.product3 += this.itemList[i]['productList'][2]['userCount']
						this.product4 += this.itemList[i]['productList'][3]['userCount']
						this.product5 += this.itemList[i]['productList'][4]['userCount']
						this.totalCount += this.itemList[i]['sumCount']
			...
			downloadExcel(){
				...
				XLSX.utils.book_append_sheet(workBook, excelData, '공급일별농가현황');  // 시트 명명, 데이터 지정
				XLSX.writeFile(workBook, this.searchSDT.replace('-','')+'_공급일별농가현황.xlsx'); // 엑셀파일 만듬

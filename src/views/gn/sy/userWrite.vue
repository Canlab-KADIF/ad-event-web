							...
							<div class="ibox-content">
								...
								<div class="form-group">
									...
									<div class="col-sm-4">
										...
										<p class="help-block"><small>영문/영문+숫자 조합으로 입력해 주세요</small></p>
									</div>
									<label for="passwordRe" class="col-sm-1 control-label"
										><span class="red">*</span>&nbsp;비밀번호 확인</label
									>
									<div class="col-sm-4">
										<input
											type="password"
...
<script>
import { GetGnCheckDuplicate, PostSyUser } from '@/api/index';
...
export default {
	...
	methods: {
		...
		fnSaved() {
			...
			if (!this.chkUserIdFlag) {
				this.$toastr.e('사용자ID 중복확인을 해주세요', '입력 오류');
				this.$refs.userId.focus();
				return false;
			}
			if (this.chkUserIdFlag && this.userId !== this.chkUserId) {
				this.$toastr.e('사용자ID 중복확인을 해주세요', '입력 오류');
				this.$refs.userId.focus();
				return false;
			}
			...
			if (this.password == '') {
				this.$toastr.e('비밀번호를 입력해 주세요', '입력 오류');
				this.$refs.password.focus();
				return false;
			}
			if (this.passwordRe == '') {
				this.$toastr.e('비밀번호 확인을 입력해 주세요', '입력 오류');
				this.$refs.passwordRe.focus();
				return false;
			}
			if (this.password !== this.passwordRe) {
				this.$toastr.e('비밀번호를 동일하게 입력해 주세요', '입력 오류');
				this.$refs.passwordRe.focus();
				return false;
			}
